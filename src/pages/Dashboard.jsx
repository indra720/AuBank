...
function Dashboard() {
  ...
  const [currentCustomer] = useState({
    ...
    id: "V-KYC-952810",
    dob: "12/05/1988",
    address: "Flat 402, Sunshine Apartments, Sector 15, Dwarka, New Delhi - 110075",
  });
            ...
            {["Console", "Reports", "Compliance"].map((item) => (
              <button 
                ...
                className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${
                  item === "Console" ? "bg-white text-orange-600 shadow-sm" : "text-gray-500 hover:text-gray-900"
                }`}
              >
                {item}
              </button>
        ...
        `}>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            ...
            {[
              ...
              <div key={i} className="p-3 rounded-xl border border-gray-100 bg-white hover:border-orange-200 transition-all cursor-pointer group">
                <div className="flex justify-between items-start">
                  <h4 className="font-semibold text-gray-800 text-sm group-hover:text-orange-600">{item.name}</h4>
                  <span className="text-[10px] text-gray-400 font-medium">{item.time}</span>
                </div>
                <p className="text-[11px] text-gray-500 mt-0.5">ID: {item.id}</p>
              </div>
            ))}
          </div>
            ...
            <div className="flex-1 bg-gray-900 rounded-2xl lg:rounded-[2.5rem] relative overflow-hidden shadow-2xl border-4 border-white">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-sm font-medium">Waiting for customer video...</p>
                </div>
              </div>
              ...
              <div className="absolute top-4 right-4 lg:top-8 lg:right-8 w-28 h-40 lg:w-44 lg:h-60 bg-gray-800 rounded-2xl border-2 border-white/10 shadow-2xl overflow-hidden backdrop-blur-sm">
                 <div className="h-full flex flex-col items-center justify-center text-gray-500 bg-gray-800/80">
                    <Mic size={24} className="mb-2" />
                    <span className="text-[10px] uppercase font-bold tracking-widest opacity-60">Agent</span>
                 </div>
              </div>
        ...
        `}>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            <div className="space-y-6">
              <DataRow label="Customer Full Name" value={currentCustomer.name} matched />
              <DataRow label="Date of Birth" value={currentCustomer.dob} matched />
            ...
            <div className="mt-8 space-y-4">
              <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl flex gap-3 shadow-sm shadow-amber-900/5">
                ...
                <p className="text-[11px] text-amber-800 font-medium">Secondary verification recommended (Poor lighting).</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <button className="py-3.5 bg-white border border-red-100 text-red-600 rounded-xl text-xs font-bold hover:bg-red-50 transition-colors">Reject</button>
                <button className="py-3.5 bg-green-600 text-white rounded-xl text-xs font-bold hover:bg-green-700 shadow-lg shadow-green-900/10 transition-colors">Approve</button>
              </div>
...
function CheckItem({ label, checked }) {
  return (
    <label className="flex items-center gap-4 cursor-pointer group">
      <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${checked ? "bg-orange-600 border-orange-600 shadow-md shadow-orange-900/10" : "border-gray-200 bg-white group-hover:border-orange-200"}`}>
        {checked && <CheckCircle2 size={14} className="text-white" />}
      </div>
      <span className={`text-[13px] font-semibold transition-colors ${checked ? "text-gray-900" : "text-gray-500 group-hover:text-gray-700"}`}>
        {label}
      </span>
    </label>
  );
