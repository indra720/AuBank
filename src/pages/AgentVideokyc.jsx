...
function AgentVideokyc() {
  ...
  const handleCapture = async (label) => {
    if (!customerVideoRef.current || !roomId) {
      toast.error("No active session to capture.");
      return;
    }
  ...
  const startWebRTC = async (stream, roomId) => {
    ...
    ws.onclose = (event) => {
      if (event.code === 4001) toast.error("Session expired or invalid token.");
    };
  };
      ...
      try {
        ...
        selfieSegmentation.onResults((results) => {
          ...
          if (logoRef.current) {
            ...
            const ratio = Math.max(canvasElement.width / logo.width, canvasElement.height / logo.height);
            const w = logo.width * ratio;
            const h = logo.height * ratio;
            canvasCtx.drawImage(logo, (canvasElement.width - w) / 2, (canvasElement.height - h) / 2, w, h);
          } else {
  ...
  useEffect(() => {
    ...
    const fetchCustomerData = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/kyc/customer-details/${roomId}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className={`w-full lg:w-72 bg-white border-r border-gray-200 flex flex-col shrink-0 ${activeTab === "queue" ? "flex" : "hidden lg:flex"}`}>
          <div className="p-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <button onClick={fetchPendingKycRequests} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-orange-600 transition-all">
              <RefreshCw size={16} />
            </button>
            <button
              onClick={async () => {
                try {
                  const agentToken = localStorage.getItem("access_token");
                  const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/kyc/clear-all`, {
                    method: "DELETE",
                    headers: { Authorization: `Bearer ${agentToken}` },
                  });
                  if (response.status === 401) {
            ...
            {callStarted && currentCustomer ? (
              <div className="bg-orange-50 border border-orange-200 rounded-xl p-3">
                <div className="flex justify-between items-start text-[10px] font-bold">
                  <span className="text-orange-600 uppercase">Current Session</span>
                  <span className="text-green-700 bg-green-100 px-1.5 py-0.5 rounded">Active</span>
                </div>
                <h3 className="font-bold text-gray-900 mt-1">{currentCustomer.name}</h3>
                <p className="text-[11px] text-gray-600">Room ID: {currentCustomer.room_id}</p>
              </div>
            ) : (
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 text-center text-gray-500 text-xs">No active session.</div>
            )}

            {pendingKycRequests.map((item) => (
              <div key={item.room_id} className={`p-3 rounded-xl border transition-all ${roomId === item.room_id ? "bg-orange-50 border-orange-200" : "border-gray-100 bg-white"}`}>
                <h4 className="font-semibold text-sm">KYC Request: {item.customer_id || item.room_id}</h4>
                <div className="flex gap-2 mt-3">
                  <button
                    onClick={async (e) => {
                      ...
                      try {
                        const agentToken = localStorage.getItem("access_token");
                        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/kyc/accept/${item.room_id}`, {
                          method: "POST",
                          headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${agentToken}`,
                        ...
                        setCallStarted(true);
                        setCurrentCustomer({ name: item.customer_id ? `Customer ${item.customer_id}` : "Guest", ...item });
                        setActiveTab("call");
                      } catch (error) {
                        toast.error("Failed to accept.");
                      }
                    }}
                    className="flex-1 py-1.5 bg-green-600 text-white text-[10px] font-bold rounded-lg"
                  >
                    Accept
                  </button>
                  <button
                    onClick={() => setPendingKycRequests((prev) => prev.filter((r) => r.room_id !== item.room_id))}
                    className="flex-1 py-1.5 bg-gray-100 text-gray-500 text-[10px] font-bold rounded-lg"
                  >
                    Reject
                  </button>
                </div>
        ...
        <main className={`flex-1 flex flex-col min-w-0 bg-gray-100 ${activeTab === "call" ? "flex" : "hidden lg:flex"}`}>
            ...
            <div className="flex-1 bg-gray-900 rounded-2xl lg:rounded-[2.5rem] relative overflow-hidden shadow-2xl border-4 border-white">
              <div className="absolute inset-0 flex items-center justify-center">
                {!callStarted ? (
                  ...
                  <video ref={customerVideoRef} autoPlay playsInline className="w-full h-full object-cover" />
                )}
                {instruction && (
              ...
              <div className="absolute top-4 right-4 lg:top-8 lg:right-8 w-28 h-40 lg:w-44 lg:h-60 bg-gray-800 rounded-2xl border-2 border-white/10 shadow-2xl overflow-hidden backdrop-blur-sm z-20">
                <video ref={agentVideoRef} autoPlay muted playsInline className="w-full h-full object-cover" />
              </div>
              <div className="absolute bottom-6 lg:bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-white/5 backdrop-blur-2xl p-3 lg:p-4 rounded-[2.5rem] border border-white/10 shadow-2xl z-30">
                <ControlButton icon={<CameraIcon size={20} />} label={isCapturing ? "..." : "Capture"} onClick={() => handleCapture("Customer Face")} />
                <ControlButton icon={<FileCheck size={20} />} label={isCapturing ? "..." : "PAN"} onClick={() => handleCapture("PAN Card")} />
                ...
                <button onClick={endCall} className="w-12 h-12 lg:w-16 lg:h-16 bg-red-500 text-white rounded-full flex items-center justify-center shadow-xl hover:rotate-90 transition-all">
                  <PhoneOff size={24} />
                </button>
              </div>
            </div>
        ...
        <aside className={`w-full lg:w-80 bg-white border-l border-gray-200 flex flex-col shrink-0 ${activeTab === "data" ? "flex" : "hidden lg:flex"}`}>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            <div className="space-y-6">
              <DataRow label="Full Name" value={data?.full_name || "N/A"} matched={!!data?.full_name} />
              <DataRow label="Mobile Number" value={data?.mobile_number || "N/A"} matched={!!data?.mobile_number} />
              <DataRow label="Aadhar Number" value={data?.aadhar_number || "N/A"} matched={!!data?.aadhar_number} />
              <DataRow label="PAN Number" value={data?.pan_number || "N/A"} matched={!!data?.pan_number} />
              <DataRow label="Date of Birth" value={data?.dob || "N/A"} matched={!!data?.dob} />
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Address (Aadhaar)</p>
                <div className="text-xs text-gray-700 font-semibold bg-gray-50 p-4 rounded-xl border border-gray-100">{data?.address || "N/A"}</div>
              </div>
              <div className="pt-2">
                <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-4">Mandatory Checklist</h3>
                <div className="space-y-3.5">
                  <CheckItem label="Mobile Verified" checked={data?.is_mobile_verified ?? false} />
                  <CheckItem label="Aadhaar Verified" checked={data?.is_aadhar_verified ?? false} />
                  <CheckItem label="PAN Verified" checked={data?.is_pan_verified ?? false} />
                </div>
              </div>
            </div>
            <div className="mt-8 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <button
                  className="py-3.5 bg-white border border-red-100 text-red-600 rounded-xl text-xs font-bold transition-colors"
                  onClick={() => roomId && setShowRejectDialog(true)}
                >
                  Reject
                </button>
                <button
                  onClick={async () => {
                    ...
                    try {
                      const agentToken = localStorage.getItem("access_token");
                      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/agent/service/decision`, {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                          Authorization: `Bearer ${agentToken}`,
                        },
                        body: JSON.stringify({ room_id: roomId, status: "Approved", notes: "" }),
                      });
                      if (response.status === 401) {
                        toast.error("Session expired.");
                        navigate("/login");
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowPopup(false)} className="p-2 hover:bg-gray-200 rounded-full">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6 bg-white flex gap-3">
              <button onClick={() => setShowPopup(false)} className="flex-1 py-4 text-gray-500 font-bold text-sm">Discard</button>
              <button onClick={() => setShowPopup(false)} className="flex-0.5 py-4 bg-orange-600 text-white font-bold text-sm rounded-2xl">Save & Continue</button>
            </div>
          </div>
        </div>
      )}
          ...
          onSubmit={async (notes) => {
            ...
            try {
              ...
              const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/agent/service/decision`, {
                ...
                body: JSON.stringify({ room_id: roomId, status: "Rejected", notes: notes }),
              });
              if (response.status === 401) {
                toast.error("Session expired.");
                navigate("/login");
  ...
  return (
    <div className="fixed inset-0 z-102 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
        ...
        <div className="p-6">
          <label htmlFor="rejectionNotes" className="block text-sm font-medium text-gray-700 mb-2">Reason for Rejection</label>
          <textarea
            id="rejectionNotes"
            className="w-full p-3 border border-gray-300 rounded-xl focus:ring-orange-500 focus:border-orange-500 text-sm"
            rows="4"
        ...
        <div className="p-4 bg-gray-50 flex justify-end gap-3">
          <button onClick={onCancel} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg text-sm font-semibold">Cancel</button>
          <button onClick={() => onSubmit(notes)} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-semibold">Confirm Reject</button>
        </div>
      </div>
    </div>
  );
...
function ControlButton({ icon, label, onClick, active }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-2 text-white/70 hover:text-white transition-all group">
      <div className={`w-10 h-10 lg:w-12 lg:h-12 ${active ? "bg-red-500" : "bg-white/10 group-hover:bg-white/20"} rounded-xl flex items-center justify-center border border-white/5`}>
        {icon}
      </div>
  ...
  return (
    <label className="flex justify-between items-center cursor-pointer group">
      <div className="flex items-center gap-4">
        <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${checked ? "bg-orange-600 border-orange-600" : "border-gray-200 bg-white"}`}>
